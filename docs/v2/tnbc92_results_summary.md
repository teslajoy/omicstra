...
**subarray funnel** (the 282 -> 35,594 trace): 282 samples -> 281 with ST counts (CN26/D1 excluded, 8 selection spots) -> 260 H&E+ST co-registered (Wang) -> 249 with successful Virchow2 niche ∩ Novae scale-embedding extraction (`eda_summary.json: scale_embeddings.matched.n_subarrays`) -> 208,786 niches after v3 gpath2vec intersection -> 35,594 test niches (14 patients, 38 subarrays). 260 is the cohort header; 249 is the post-embedding intersection; 208,786 is the final input to the alignment grid. all refer to the same funnel.
...
| **Observed result** | R4 AUC 0.859, CKA 0.631 (exceeds proposal range). R1 / R5 AUC 0.761 (top of range). R6 AUC 0.737 (in range). R2 0.725 (in range). R3 0.674 (slightly below range). Among classical baselines: B2 Procrustes 0.706, B1 CCA 0.544, B3 Unaligned PCA 0.440. B4 random-init at chance (0.496). 5 of 6 contrastive runs (R4, R1, R5, R6, R2) beat the strongest classical baseline B2 with non-overlapping 95% CIs. R3 Barlow Twins is the lone exception - sits below B2 with a 0.026 CI gap (see `## 95% confidence intervals` section). |
...
| **Notes** | DAG-resolution H3 decomposition (395 sub-pathway nodes; v1 z_he cross-patient one-sided BH-FDR<0.05: **R4 317/395 sig = 80.3%**, B2/B3 268/395 = 67.8%, B1 248/395 = 62.8%, late-fusion contrastive runs 1.5-3.8%; biologically distinct top hits include BTLA z=6.92, Integrin z=6.21, Collagen z=5.22-5.28) is deferred for v3. v1 numbers preserved at [`../v1/tnbc92_results_summary.md`](../v1/tnbc92_results_summary.md). |
...
## 95% confidence intervals (added 2026-06-02)

today's CI computation surfaces uncertainty on the three quantitative panels of the scorecard. point estimates above are unchanged; CIs strengthen the rigor of the comparisons. raw plot-ready arrays per run: `runs/tnbc-92_v3/{run}/h1_eval_data.npz`. method notes at the foot of each table.

### H1 - AUC + CKA (Hanley-McNeil for AUC; bootstrap for CKA)

| run | AUC [95% CI] | CKA [95% CI] |
|---|---|---|
| **R4** | **0.859 [0.857, 0.862]** | **0.631 [0.626, 0.636]** |
| R1 | 0.761 [0.758, 0.764] | 0.312 [0.305, 0.318] |
| R5 | 0.761 [0.758, 0.764] | 0.314 [0.306, 0.320] |
| R6 | 0.737 [0.734, 0.740] | 0.239 [0.233, 0.245] |
| R2 | 0.725 [0.722, 0.728] | 0.101 [0.098, 0.105] |
| B2 | 0.706 [0.703, 0.710] | 0.125 [0.122, 0.129] |
| R3 | 0.674 [0.671, 0.677] | 0.306 [0.299, 0.312] |
| B1 | 0.544 [0.541, 0.547] | 0.076 *(CI n/a)* |
| B4 | 0.496 [0.493, 0.499] | 0.120 [0.117, 0.124] |
| B3 | 0.440 [0.437, 0.443] | 0.125 [0.122, 0.129] |

method: AUC 95% CI via Hanley & McNeil (1982) closed-form (matched to nonparametric bootstrap within 3 decimals); n_pos = 35,594, n_neg = 355,940 (10:1 sampled). CKA 95% CI via nonparametric bootstrap over test niches, B=200, seed=42. B1 CKA CI is not estimable by standard bootstrap because CCA's near-diagonal cross-covariance structure produces a structurally upward-biased bootstrap distribution; the point estimate is correct. source: `scripts/eval_h1_bootstrap_ci.py`, output `runs/tnbc-92_v3/{run}/metrics_h1_ci.json`.

CI-derived observations: R3 Barlow Twins [0.671, 0.677] sits below B2 Procrustes [0.703, 0.710] with a 0.026 non-overlapping CI gap - the one contrastive run that loses to the strongest classical baseline. B4 random-init CI [0.493, 0.499] explicitly straddles 0.500, confirming chance. B3 unaligned-PCA CI [0.437, 0.443] sits entirely below 0.500, confirming below-chance (matched-pair cosine anti-correlation in unaligned PCA space).

### H2 Part A - mc_megacluster KMeans ARI (patient-level bootstrap)

| run | ARI | 95% CI |
|---|---|---|
| **R5** | **0.246** | [0.213, 0.424] |
| R6 | 0.244 | [0.202, 0.411] |
| R1 | 0.234 | [0.203, 0.396] |
| B1 | 0.211 | [0.191, 0.458] |
| B2 | 0.194 | [0.145, 0.482] |
| B3 | 0.194 | [0.145, 0.482] |
| B4 | 0.189 | [0.134, 0.443] |
| R3 | 0.159 | [0.144, 0.237] |
| R2 | 0.152 | [0.100, 0.279] |
| R4 | 0.101 | [0.094, 0.154] |

method: patient-level nonparametric bootstrap (resample 14 test patients with replacement, B=200, seed=42), KMeans(k=14, n_init=10, seed=42) fit once per run on the full test set; ARI recomputed on (true, pred) labels of each resampled niche subset. source: `scripts/eval_h2_bootstrap_ci.py`, output `runs/tnbc-92_v3/eval/H2/metrics_h2_ci.json`.

CI-derived observations: the R5/R6/R1 3-way tie at the top is statistically real - CIs overlap substantially in [0.20, 0.42]. R4 H2-A bottom-of-grid is robust - CI [0.094, 0.154] sits clean below every other run. B4 random-init ARI 0.189 [0.134, 0.443] is non-trivial - KMeans on Xavier-projected Virchow2 + novae + gpath2vec still picks up mc structure, because the input features carry mc-relevant signal before any contrastive training. R2 SupCon (0.152) and R3 Barlow (0.159) sit *below* B4 random-init on the point estimate, with overlapping CIs - their contrastive losses did not improve over no training on this metric.

### H2 Part C - bio/patient ratio (delta-method)

| run | ratio | 95% CI |
|---|---|---|
| **R6** | **0.547** | [0.476, 0.617] |
| R5 | 0.497 | [0.428, 0.566] |
| R1 | 0.461 | [0.394, 0.528] |
| R4 | 0.376 | [0.279, 0.473] |
| R2 | 0.372 | [0.285, 0.459] |
| R3 | 0.250 | [0.158, 0.343] |
| B1 | 0.071 | [0.053, 0.088] |
| B2/B3/B4 | (n/a) | not in eval_alignment_biology pipeline (no per-niche biology.parquet for classical / random-init runs) |

method: Fieller-style delta-method 95% CI on the ratio TIME_z / patient_id_z, using existing biology.parquet permutation-derived z-scores and unit-variance approximation. SE(ratio) ≈ |ratio| × sqrt(1/z_TIME² + 1/z_patient²). source: `scripts/eval_h2_bootstrap_ci.py`, same output JSON.

CI-derived observations: B1's catastrophic ratio is statistically rock-solid - CI [0.053, 0.088] sits entirely below the raw_he floor of 0.137. The R6/R5/R1 top tier overlaps in [0.39, 0.62]. R3's CI [0.158, 0.343] overlaps R4's [0.279, 0.473] - those two are not statistically distinguishable on this metric.
