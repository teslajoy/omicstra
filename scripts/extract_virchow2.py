"""
extract_virchow2.py - Virchow2 embeddings at multiple spatial scales.

one forward pass per patch, saves different representations by --level:
  cell:  CLS token per spot's own patch -> (n_spots, 1280)
  niche: mean pool CLS of k=6 spatial neighbors -> (n_spots, 1280)

patch tokens (256 x 1280 per patch) not saved by default (~370GB).
recompute from raw patches for early interaction experiments when needed.

boundary spots with fewer than k neighbors use available neighbors
(don't discard real tissue, log neighbor_count in meta).

usage:
    python scripts/extract_virchow2.py --level cell
    python scripts/extract_virchow2.py --level niche
    python scripts/extract_virchow2.py --level cell niche   # both in one pass
    python scripts/extract_virchow2.py --step check

outputs:
    data/embeddings/virchow2_cell/{subarray_id}.npy        - (n_spots, 1280)
    data/embeddings/virchow2_niche/{subarray_id}.npy       - (n_spots, 1280)
    data/embeddings/virchow2_niche/{subarray_id}_meta.tsv  - coords + neighbor count
"""
...
def sanity_check_neighbors(spots, key):
    ...
    expected inter-spot distance: ~158px (150um / ~0.95 um/px).
    acceptable range: 100-300px."""
    ...
    print(f'  neighbor check ({key}): median={median_dist:.1f}px, '
          f'min={nn_dists.min():.1f}px, max={nn_dists.max():.1f}px (HE-small space)')
    ...
    for i in range(min(5, len(coords))):
        ...
        if len(neighbor_dists) > 0 and not all(50 < d < 500 for d in neighbor_dists):
            print(f'  WARNING: spot {i} unexpected neighbor distances: '
                  f'{neighbor_dists.tolist()}')

    return median_dist


def run_extraction(subarrays, levels):
    """extract Virchow2 embeddings at requested levels."""
    import timm
    ...
    do_cell = 'cell' in levels
    do_niche = 'niche' in levels

    # setup output dirs
    cell_dir = EMB_DIR / 'virchow2_cell'
    niche_dir = EMB_DIR / 'virchow2_niche'
    if do_cell:
        cell_dir.mkdir(parents=True, exist_ok=True)
    if do_niche:
        niche_dir.mkdir(parents=True, exist_ok=True)

    # skip completed - a subarray is done when all requested levels have files
    done = set()
    for s in subarrays:
        key = s['key']
        cell_done = (not do_cell) or (cell_dir / f'{key}.npy').exists()
        niche_done = (not do_niche) or (niche_dir / f'{key}.npy').exists()
        if cell_done and niche_done:
            done.add(key)

    todo = [s for s in subarrays if s['hd_path'] and s['key'] not in done]
    no_hd = sum(1 for s in subarrays if not s['hd_path'])
    print(f'Virchow2 [{"+".join(levels)}]: {len(done)} done, {len(todo)} remaining, '
          f'{no_hd} no HD')
    ...
    for i, sub in enumerate(todo):
        ...
        for j in range(0, len(patches), batch_size):
            ...
            with torch.no_grad():
                out = model(batch)
            # CLS token is index 0, skip register tokens (1-4), patch tokens (5-260)
            cls_tokens.append(out[:, 0, :].cpu().numpy())
        cls_all = np.vstack(cls_tokens)  # (n_spots, 1280)

        # --- cell level: CLS per spot's own patch ---
        if do_cell:
            np.save(cell_dir / f'{key}.npy', cls_all.astype(np.float32))

        # --- niche level: mean pool k=6 neighbors ---
        if do_niche:
            coords = spots[['pixel_x', 'pixel_y']].values
            tree = KDTree(coords)
            k_query = min(K_NEIGHBORS + 1, n_spots)
            ...
            np.save(niche_dir / f'{key}.npy', niche_embs)

            meta = spots[['pixel_x', 'pixel_y']].copy()
            meta['neighbor_count'] = neighbor_counts
            meta.to_csv(niche_dir / f'{key}_meta.tsv', sep='\t')

        # progress
        elapsed = time.time() - t0
        rate = (i + 1) / elapsed * 3600
        remaining = (len(todo) - i - 1) / rate * 3600 if rate > 0 else 0
        parts = [f'{n_spots} spots']
        if do_cell:
            parts.append(f'cell {cls_all.shape}')
        if do_niche:
            boundary = (neighbor_counts < K_NEIGHBORS + 1).sum()
            parts.append(f'niche {niche_embs.shape} ({boundary} boundary)')
        print(f'  [{i+1}/{len(todo)}] {key}: {", ".join(parts)} '
              f'[{elapsed/60:.0f}m, ~{remaining/60:.0f}m remaining]')

        del patches, cls_tokens, cls_all

    print(f'Virchow2 done: {time.time() - t0:.0f}s total')


def check_progress():
    """report what's done for each level."""
    for name in ['virchow2_cell', 'virchow2_niche']:
        d = EMB_DIR / name
        if d.exists():
            files = [f for f in d.glob('*.npy')]
            if files:
                total_spots = sum(np.load(f).shape[0] for f in files)
                extra = ''
                if name == 'virchow2_niche':
                    low = 0
                    for f in files:
                        meta_path = d / f'{f.stem}_meta.tsv'
                        if meta_path.exists():
                            meta = pd.read_csv(meta_path, sep='\t', index_col=0)
                            low += (meta['neighbor_count'] < 4).sum()
                    extra = f', {low} with <4 neighbors'
                print(f'{name}: {len(files)} subarrays, {total_spots} spots{extra}')
            else:
                print(f'{name}: no files yet')
        else:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--level', nargs='+', choices=['cell', 'niche'],
                        default=['cell', 'niche'],
                        help='extraction level(s): cell, niche, or both (default: both)')
    ...
    if args.step == 'check':
        ...
        run_extraction(subarrays, args.level)
