"""
compare_cell_niche_qc.py - QC comparison of Virchow2 cell vs niche.

mirrors compare_encoders_qc.py logic against mc_labels + morphology_labels
to decide whether cell-scale or niche-scale is the better primary H&E
encoder for the alignment task.

metrics (same as compare_encoders_qc.py):
...
output: comparison table + decision gate for cell vs niche.
"""
...
print('\n=== decision gate: cell vs niche ===')
cell = results.get('virchow2_cell', {})
niche = results.get('virchow2_niche', {})

if cell and niche:
    lp_delta = cell.get('lp_mc', 0) - niche.get('lp_mc', 0)
    cs_delta = cell.get('cross_sil_mc', 0) - niche.get('cross_sil_mc', 0)
    ber_delta = cell.get('batch_ratio', 0) - niche.get('batch_ratio', 0)

    print(f'  lp_mc       cell - niche = {lp_delta:+.3%}')
    print(f'  cross_sil   cell - niche = {cs_delta:+.4f}')
    print(f'  batch_ratio cell - niche = {ber_delta:+.3f}  (lower is better)')

    wins = 0
    if lp_delta > 0: wins += 1
    if cs_delta > 0: wins += 1
    if ber_delta < 0: wins += 1
    winner = 'virchow2_cell' if wins >= 2 else 'virchow2_niche'
    print(f'  -> winner on 2/3+ metrics: {winner}')

import json
out_path = BIO_DIR / 'encoder_qc_cell_vs_niche.json'
