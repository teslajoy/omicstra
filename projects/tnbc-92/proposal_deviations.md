...
## data, encoder and architecture facts against the proposal (appended 2026-09-15)

the proposal's description of the dataset and the ST encoder was written before the data was opened. each row quotes the proposal, states what the data and code are, and names where that is verified. none changes a result; each changes a sentence a reader would otherwise take from the proposal.

| proposal says | as used | verified against |
|---|---|---|
| "co-registered Visium spatial transcriptomics" | original Spatial Transcriptomics (Stahl 2016): 1934 spots per array, 100 um diameter, 150 um centre-to-centre. not Visium | Wang 2024 Methods; `projects/tnbc-92/platform.json#platforms.original_st` |
| "Estimated >18,000 co-registered spatial spots passing QC" | four counts, four populations. **270,310** spots: the source study, 281 subarrays, after its three spot filters (537,069 -> 281,852 out-of-tissue and >1% artefact removed -> minus 11,542 under 500 UMIs). **286,250** spots: this project's canonical ingest, 280 samples, no spot filter applied - 11,375 of them are under 500 UMIs, so the study's UMI filter alone would leave 274,875; the remaining 4,565 against 270,310 is unexplained: the study's out-of-tissue and artefact selection is not reproducible from the published description (its UMI filter is - 11,375 here against 11,542 there). **275,917** niches: the join before the pathway-coverage drop, 259 subarrays with both modalities and a gpath2vec build. **208,786** niches: after that drop, the table every run reads | Wang 2024 Results and Methods (spot selection); `projects/tnbc-92/data/canonical/ingest.json`; `data/embeddings/niches_v3/manifest.json` |
| "17 expert-annotated morphological categories" | 15 histomorphological categories. the per-spot annotation matrix carries 18 columns = 15 tissue classes + Nothing, Artefacts, Hole | Wang 2024 Results; `byArray/*/*/annotBySpot.RDS` |
| "Novae ... 256-dimensional expression embeddings"; "ST 256 to 512" | 64-d `novae_latent` from `MICS-Lab/novae-human-0`. the ST projection input is 576-d (novae 64 + gpath2vec 512) for every run except R6 (64-d) | `notebooks/embeddings/novae_st_embeddings.ipynb` cell 18; `configs/v3/*.json`; `MODELS.md` |
| "H&E 1536 to 512" | Virchow2 1280 to 512. UNI2 (1536) is the declared alternative, not run on the grid - see the H1 encoder-sensitivity row above | `configs/v3/*.json`; `MODELS.md` |
| a model inside the orchestration for routing and hypothesis evaluation | the model moved to the client; routing is a deterministic rule over evidence and the server holds no model | `README.md` section "declared deviation from the proposal" |

"92 patients" is consistent with the source: 281 of 288 subarrays passed sequencing QC, covering 92 of 94 patients.
